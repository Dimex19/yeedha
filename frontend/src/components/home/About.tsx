...
const Download = ({text1, text2, title, description, cta}: AboutBlock) => {
  ...
  return (
    <div className="font-[Manrope] bg-[#2563EB] text-white text-center pt-[66px] pb-[85px]">
        <div className="md:w-[955px] mx-auto px-5 lg:px-0">
            <p className="text-[16px] md:text-[24px] font-medium mb-[10px]">{text1}</p>
            <p className="text-[21px] md:text-4xl lg:text-[55px] mb-2 font-bold">{title}</p>
            <p className="text-[14px] md:text-2xl lg:text-[28px] font-medium mb-[20px]">{description}</p>
            <p className="text-[10px] md:text-[16px] font-medium px-5 md:w-[631px] mx-auto">{text2}</p>
            {cta?.[0] && (
          <a
            href={cta[0].href}
            target={cta[0].isExternal ? "_blank" : "_self"}
            // rel="noopener noreferrer"
            onClick={()=> {console.log("CTA clicked", cta[0].href)}}
          >
            <button className="w-40 md:w-[210px] h-[40px] lg:h-[55px] text-[18px] text-[#2563EB] bg-white font-bold rounded-[8px] lg:rounded-[12px] mt-3">
              {cta[0].label}
            </button>
          </a>
        )}
        ...
        <div className="flex mt-[57px] justify-center md:gap-x-[14.98px] mb-[-85px]">

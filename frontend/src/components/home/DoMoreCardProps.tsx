...
const DoMoreCardProps = ({ image, title, description }: DoMoreCardProp) => {
// const DoMoreCardProps = ({ image, title, description, BtnText }: DoMoreCardProp) => {
  return (
    <div className="relative mb-4 mx-2">
      ...
      {/* Content on top */}
      <div className="absolute top-1/3 md:top-2/3 pl-[30px] z-10">
        <p className="font-[Manrope] text-[20px] lg:text-3xl xl:text-[42px] font-semibold text-white">{title}</p>
        <p className="font-[Manrope] text-[9px] lg:text-[12px] xl:text-[16px] font-medium text-white max-w-[240px] md:max-w-[350px] xl:max-w-[492px] mb-2.5">{description}</p>

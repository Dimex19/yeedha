            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 justify-between items-center lg:items-start space-y-8 lg:space-y-0">
                {cardWithIcon.map((feature, index) => {
                  const imgUrl = feature?.icon?.image?.url ? getStrapiMedia(feature.icon.image.url) : "";
                    return (
                    <div key={index} className="w-full md:w-3/4 lg:w-[381px] flex flex-col justify-center items-center gap-y-6 lg:gap-y-0 mt-8 lg:mt-[50px] border border-[#D8E9FF] rounded-[20px] h-[329px] p-6 lg:p-[35px] mx-auto">
                      <div className="w-[80px] h-[80px] lg:w-[105px] lg:h-[105px] rounded-full bg-[#E3EBFF] flex items-center justify-center lg:mb-[43px]">
                        <img
                      ...
                      <p className="text-lg lg:text-[20px] text-[#2563EB] font-semibold text-center lg:mb-[17px]">{feature.title}</p>
                      <p className="w-full max-w-xs lg:w-[292px] text-[#6C737F] text-sm lg:text-[14px] text-center">{feature.description}</p>
                </div>)})}
            </div>
        </div>
        {/* <StartAndNewsletter/> */}
